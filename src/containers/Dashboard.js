...
export default class {
  ...
  handleEditTicket(e, bill, bills) {
    ...
    if (e.currentTarget.className.includes("selectCard")) {
      ...
      <h3> Validations </h3>
      <section class="container-big-billed-icon" ><span id="big-billed-icon" data-testid="big-billed-icon"> ${BigBilledIcon} </span></section>
      `);
      $("#arrow-icon1").click((e) => this.handleShowTickets(e, bills, 1));
      $("#arrow-icon2").click((e) => this.handleShowTickets(e, bills, 2));
      $("#arrow-icon3").click((e) => this.handleShowTickets(e, bills, 3));
      $(`#open-bill${bill.id}`).removeClass("selectCard");
    }

    if (this.counter % 2 === 0) {
      bills.forEach((b) => {
        $(`#open-bill${b.id}`).css({ background: "#0D5AE5" }); // bleu
        $(`#open-bill${b.id}`).removeClass("selectCard");
      });
  ...
  handleShowTickets(e, bills, index) {
    ...
    bills.forEach((bill) => {
      // Ajout des écouteurs d'événement uniquement si cet élément n'a pas d'écouteur de données
